...
| ACL-01 | Workspace, channel, user, profile, repository, tool, memory, and task authority are consistent at every transition. | Table-driven denial suite with audit records. | PENDING: Slack event/action/ambient, memory, and schedule workspace/user/profile/task denials pass; tool and isolation transitions remain |
...
| OPS-01 | Fresh install, onboarding, diagnostics, background service, upgrade/migration, backup/restore, log redaction, and uninstall work. | Clean-host script and captured run log. | PENDING: foreground service, live doctor path, signal shutdown, migrations, schedule operator controls, non-destructive backup/restore, and structured log-redaction fixtures exist; clean install, service-manager install, upgrade, and uninstall remain |
