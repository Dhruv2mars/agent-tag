...
| ACL-01 | Workspace, channel, user, profile, repository, tool, memory, and task authority are consistent at every transition. | Table-driven denial suite with audit records. | PENDING: Slack event and action workspace/channel/user/route/task/thread/interaction denials pass; memory, tool, and isolation transitions remain |
...
| AUD-01 | Every state-changing action has actor, authority, source, target, result, and correlation IDs. | Queryable audit export from acceptance run. | PENDING: paginated NDJSON export and durable core transition rows pass without message bodies; remaining feature transitions and acceptance-run export remain |
