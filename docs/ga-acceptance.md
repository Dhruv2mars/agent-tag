...
| T3-02 | Stable command IDs make dispatch replay safe and receipts reconcile after bridge restart. | Crash-point integration test against real T3. | PENDING: real command replay returns one receipt sequence; bridge restart remains |
...
| T3-04 | Provider/model selection is explicit and sticky; unsupported capabilities fail clearly. | Capability matrix against at least two real providers. | PENDING: Codex and Claude are authenticated and ready in the live T3 catalog |
