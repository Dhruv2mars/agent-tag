...
| SLK-02 | Repeated and overlapping Slack deliveries create one task and one user turn. | Automated retry fixture plus live retry where practical. | PENDING |
...
| T3-01 | The adapter authenticates to pinned T3 `0.0.42` with only required scopes. | Executable compatibility test against release binary. | PENDING |
| T3-02 | Stable command IDs make dispatch replay safe and receipts reconcile after bridge restart. | Crash-point integration test against real T3. | PENDING |
...
| T3-04 | Provider/model selection is explicit and sticky; unsupported capabilities fail clearly. | Capability matrix against at least two real providers. | BLOCKED: provider authentication |
...
| REL-01 | In-task serialization, global concurrency bounds, durable outbox, and recovery prevent loss or blind replay. | Deterministic fault-injection suite. | PENDING |
...
| AUD-01 | Every state-changing action has actor, authority, source, target, result, and correlation IDs. | Queryable audit export from acceptance run. | PENDING |
