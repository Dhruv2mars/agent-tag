...
| OPS-01 | Fresh install, onboarding, diagnostics, background service, upgrade/migration, backup/restore, log redaction, and uninstall work. | Clean-host script and captured run log. | PENDING: foreground service, live doctor path, signal shutdown, migrations, and structured log-redaction fixture exist; clean install, service-manager install, backup/restore, upgrade, and uninstall remain |
