...
It scanned 66 files and 448,156 bytes, skipped no symbolic links, and found no exact or known-pattern credentials. The scanner omits `.git` and `node_modules`; it scans ignored files such as `.env`, build output, SQLite files, prompts, and artifacts when those files are under a requested root.
