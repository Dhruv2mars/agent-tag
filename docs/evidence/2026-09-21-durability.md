...
`bun run check` passed with 15 tests and one intentionally disabled live-T3 suite. The store fixtures proved:
...
4. A Slack outbox message survived close/reopen, could not be stolen before lease expiry, and was reclaimed afterward with the same client message ID.
